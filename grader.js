...
var APPURL_DEFAULT = "https://secret-woodland-6547.herokuapp.com/"; 
...
var getHtml = function(result, response) {
	...
	//console.log('Output: ' + util.format(result));
	var outfile = "recvhtml.html";
...
if(require.main == module) {
	...
	.parse(process.argv);
    if(program.url) {
	console.log("Checking HTML Tags from url: " + program.url);
	rest.get(program.url).on('complete', getHtml);
	var checkJson = checkHtmlFile("recvhtml.html", program.checks);
	var outJson = JSON.stringify(checkJson, null, 4);
	console.log(outJson);
	var outJsonFile = "outJsonFile.json";
	fs.writeFileSync(outJsonFile, outJson);
    } else {
	console.log("Checking HTML Tags from file: " + program.file);
